...
- **The primary button's label and action.** There is no filter on `PostPublishButton`, so "Stage changes" is applied by matching the button and intercepting the click ([Gutenberg #16308](https://github.com/WordPress/gutenberg/issues/16308) is where that belongs). This one is cosmetic by construction: if the match fails, the editor says so in a notice, the click reaches core, and the server stages the save and steers the editor back into the staging flow.

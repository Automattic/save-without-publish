...
The staged copy's own screen does not offer these fields to begin with. The controls for the slug, publish date, author, template, format, discussion, featured image, categories, tags, and page attributes are taken off the Summary panel entirely, rather than left on screen only to be refused when the copy is saved — a value like the copy's own internal slug is not one an editor should have to look at, let alone remember to put back. The **Published post** row says where those settings live instead, and links there. The classic editor takes the same controls off its own screen, the same way, for the same reason.
...
**Can renaming a staged copy's slug collide with the published post's, the way it does in some other staging tools?**

No. The slug is never staged, so there is no second, editable slug to rename into a collision with the first. The staged copy's own screen does not even offer the control: what an editor sees there is the copy's own internal slug (`swpub-staged-<id>`), which is never published and never meant to be typed into. To change the URL, open the published post and change it there, the way any other slug change is made.
